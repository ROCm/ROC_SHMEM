        ...
        if (rank % 2 == 0) {
            ...
            for (win_post_i = 0; win_post_i < win_posts; win_post_i++) {
                for (win_i = 0; win_i < WINDOW_SIZE; win_i++) {
                    roc_shmem_ctx_putmem_nbi(my_ctx, my_dest_buf, my_source_buf, my_message_size, rank + 1);
                }
...
void print_usage(const char *argv0)
{
    printf("Usage:\n");
    printf
        ("  mpiexec -n 2 -ppn 1 ###options-to-bind-threads### -hosts <sender>,<receiver> %s <options>\n",
         argv0);
    printf("\n");
    printf("Options:\n");
